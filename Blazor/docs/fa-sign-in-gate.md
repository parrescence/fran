...
hand-rolling its own centered card. Wraps [FaAuthTemplate](fa-auth-template.md)
internally. FaSignInGate never performs the actual sign-in redirect itself —
`OnSignIn` is where the caller navigates to its own login route.
...
|---|---|---|
| `BrandText` | `string` | **required** — see [FaAuthTemplate](fa-auth-template.md) |
